...
main = do
  ...
  expect (Map.fromList [("x", VarRange 0 4)]) always
    [[("x",0)], [("x",1)], [("x",2)], [("x",3)]]
  -- Singleton range => just it
  expect (Map.fromList [("x", VarRange 0 1), ("y", VarRange 2 3)]) always
    [[("x",0), ("y",2)]]
  -- Contain empty range => unsat
  expect (Map.fromList [("x", VarRange 0 4), ("y", VarRange 2 2)]) always
    []
  ...
  expect (Map.fromList [("x", VarRange 0 3), ("y", VarRange 0 2)]) (conjunct ["x" %/=% "y", "x" %==% "y"])
    []
...
testGraphColoring n k edges = graphValidity >> test vars (conjunct cons)
  where
    graphValidity =
      F.for_ edges $ \(i,j) ->
